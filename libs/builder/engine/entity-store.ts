import { NgDocConfiguration, NgDocPageKeyword } from '@ng-doc/builder';
import {
  asArray,
  NgDocGlobalKeyword,
  NgDocKeyword,
...
import { formatKeywordKey, getKeywordTypeFromEntity, isRouteEntity } from '../helpers';
...
export class NgDocEntityStore extends Map<string, NgDocEntity> {
  ...
  getByKeyword(keyword: string): NgDocKeyword | undefined {
    const key: string = formatKeywordKey(keyword);
    ...
    keywords
      ...
      .forEach((keywords: Record<string, NgDocGlobalKeyword>) => {
        objectKeys(keywords).forEach((key: string) => {
          ...
          if (keyword) {
            this.globalKeywords.set(formatKeywordKey(key), {
  ...
  updateKeywordMap(): void {
    ...
    this.asArray().forEach((entity: NgDocEntity) => {
      if (isRouteEntity(entity) && entity.isReadyForBuild) {
        entity.keywords.forEach((keyword: NgDocPageKeyword) =>
          this.addKeyword(formatKeywordKey(keyword.key), {
            title: keyword.title,
            path: keyword.path,
