...
export class NgDocEntryPoint {
  ...
  async renderModule(): Promise<void> {
    ...
    await renderer.renderToFile(
      ...
      path.join(this.moduleFolder, this.moduleFileName),
    );
  }

  async renderPage(): Promise<void> {
    if (this.compiledEntryPoint) {
      const mdPath: string = path.join(this.folder, this.compiledEntryPoint.mdFile);
      const renderer: NgDocRenderer = new NgDocRenderer<NgDocPageEnv>({
        ngDoc: {
          page: this.compiledEntryPoint
        },
        ngDocDemo: (str: string) => str,
        ngDocApi: (str: string) => str,
      });

      await renderer.renderToFile(mdPath, path.join(this.moduleFolder, RENDERED_PAGE_NAME), {fullPath: true});
    }
  }
