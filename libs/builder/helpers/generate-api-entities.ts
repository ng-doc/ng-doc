...
export function generateApiEntities(
  ...
  apiRootEntity.sourceFile
    ...
    .reduce(
      (
        acc: Map<string, Map<string, NgDocSupportedDeclaration[]>>,
        [path, declarations]: [string, ReadonlyMap<string, ExportedDeclarations[]>],
      ) => {
        const current: Map<string, NgDocSupportedDeclaration[]> = acc.get(path) ?? new Map();

        declarations.forEach((value: ExportedDeclarations[], key: string) => {
          const existing: NgDocSupportedDeclaration[] | undefined = current.get(key);
          const supported: NgDocSupportedDeclaration[] = value.filter(isSupportedDeclaration);
    ...
    .forEach((declMap: Map<string, NgDocSupportedDeclaration[]>, path: string) => {
      declMap.forEach((declarations: NgDocSupportedDeclaration[], name: string) => {
        declarations.forEach((declaration: NgDocSupportedDeclaration) => {
